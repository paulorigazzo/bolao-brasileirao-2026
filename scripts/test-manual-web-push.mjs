...
assert.match(app,/data-attention-push-user/);
assert.match(app,/openAdminPushReminder\(preview,userId\)/);
assert.match(app,/Participante sem notificações ativas/);
assert.match(app,/placar\$\{me\.exact===1\?"":"es"\} exato\$\{me\.exact===1\?"":"s"\} no campeonato/);
