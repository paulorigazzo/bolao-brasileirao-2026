-- L08 — Qualifica o identificador do lote para evitar ambiguidade com participantes.id.
begin;
...
begin
  ...
  select array_agg(distinct selected.user_id) into v_ids
  ...
  if exists(
    select 1 from unnest(v_ids) as selected(user_id)
    where selected.user_id=(select auth.uid()) or not exists(
      select 1 from public.participantes p
      join public.participantes_autorizados pa on lower(pa.email)=lower(p.email)
      where p.user_id=selected.user_id and p.ativo is true and pa.ativo is true
...
revoke all on function public.adicionar_membros_liga_em_lote(uuid,uuid[]) from public,anon;
grant execute on function public.adicionar_membros_liga_em_lote(uuid,uuid[]) to authenticated;
