-- Corrige a qualificação de colunas homônimas no resumo do Ranking provisório.
...
begin
  ...
  with participantes_validos as (
    ...
    select
      count(*) filter (where jc.estado_provisorio='finished')::integer as encerrados,
      count(*) filter (where jc.estado_provisorio='live')::integer as ao_vivo,
      count(*) filter (where jc.estado_provisorio='suspended')::integer as suspensos,
      count(*) filter (where jc.estado_provisorio='postponed')::integer as adiados,
      count(*) filter (where jc.estado_provisorio='future')::integer as futuros,
      count(*) filter (where jc.estado_provisorio='cancelled')::integer as cancelados,
      max(coalesce(jc.sincronizado_em,jc.atualizado_em)) as atualizado_em
    from jogos_classificados jc
    where jc.rodada = p_rodada
  )
...
-- Rollback: reaplicar a definição anterior somente se o campo de retorno
-- atualizado_em também for renomeado para evitar a ambiguidade corrigida aqui.
